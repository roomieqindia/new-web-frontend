...
const BhojanalayListingPage = () => {
  ...
  useEffect(() => {
    const fetchOfficeData = async () => {
      try {
        const response = await axiosI.get(`/bhojnalayas`);
        const res = await axiosI.get(`/bhojnalayas/${id}`);
    ...
    <>
      ...
      <main className="w-full pt-32 md:pt-48 bg-white">
        ...
        <div className="pl-4 text-black px-4 sm:px-8 lg:px-16 text-base sm:text-xl border-b-2 pb-2 opacity-80 border-black font-normal font-['Poppins']">
          Home \ Bhojanalayas \ {officeData?.bhojanalayName}
        </div>
            ...
            <div className="bg-white rounded-lg shadow-md p-6 mt-16">
              <div className="flex flex-col md:flex-row justify-between mb-6">
                ...
                <button className="bg-pink-300 text-black px-4 py-2 mt-2 rounded-full">
                  Price of Thali: ₹{officeData?.priceOfThali}/-
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-6">
                <ul className="space-y-2">
                  <li>
                    Monthly Thali Charges 1 Time: ₹{officeData?.monthlyCharge1}
                  </li>
                  <li>
                    Monthly Thali Charges 2 Time: ₹{officeData?.monthlyCharge2}
                  </li>
                  <li>Timings: {officeData?.timings}</li>
                  <li>Special Thali: {officeData?.specialThali}</li>
                </ul>
                <ul className="space-y-2">
                  <li>Parcel of Food: {officeData?.parcelOfFood}</li>
                  <li>Veg / Non-Veg: {officeData?.veg}</li>
...
export default BhojanalayListingPage;
